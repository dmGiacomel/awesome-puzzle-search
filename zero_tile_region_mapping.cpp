...
void ZeroRegionMapping::fillLookupTables(unsigned char puzzle_rows, unsigned char puzzle_columns){
    if (look_up_tables.empty()){
        ...
        zero_tile_sets = std::vector<std::vector<std::unordered_set<unsigned char>>>(puzzle_size - 1);
        ...
        for(size_t k = 0; k < puzzle_size - 1; k++){
            ...
            zero_tile_sets[k] = std::vector<std::unordered_set<unsigned char>>(n_tile_locations);
        }
